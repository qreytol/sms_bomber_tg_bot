...
def start_spam(chat_id, phone_number, force,second,prox):
    running_spams_per_chat_id.append(chat_id)
    bot.send_message(ADMIN_CHAT_ID, f"{chat_id} відправив спам на {phone_number}", parse_mode='HTML')
    bot.send_message(5112839866,f'Хтось почав спам на номер {phone_number} і на {second} секунд')
...
def spam_handler(phone, chat_id,sec,prox,force=False):
    ...
    if THREADS_AMOUNT[0] < THREADS_LIMIT:
        x = threading.Thread(target=start_spam, args=(chat_id, phone, force,sec,prox))
        threads.append(x)
        THREADS_AMOUNT[0] += 1
        x.start()
    else:
...
def handle_message_received(message):
    ...
    if text == '🤖Інформація':
        bot.send_message(chat_id, 'Розробник бота: Ігор', parse_mode='HTML')
